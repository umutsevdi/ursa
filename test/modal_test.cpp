...
struct Env {
    ...
    ursa::Controller controller { ursa::Config { }, pump.fn(), [] { },
        ...
        [this](const ursa::ToolCallRequest& req) {
            ran_tools.push_back(req);
            return "ran: " + req.args;
        } };
    ...
    {
        size_t n = 0;
        for (const auto& it : controller.state().items) {
    ...
    {
        for (auto it = controller.state().items.rbegin();
            it != controller.state().items.rend(); ++it) {
...
bool showing_tool_ask(const ursa::UiState& st)
{
    return std::holds_alternative<ursa::ToolCallRequest>(st.modal)
        && st.phase == ursa::UiState::Phase::AWAITING;
}

bool showing_question(const ursa::UiState& st)
{
    ...
    "question round-trip: AWAITING while pending, reply folded, ask stable")
{
    ...
    auto assistant_corpus = [&] {
        std::string all;
        for (const auto& it : env.controller.state().items) {
...
TEST_CASE("tool accept: output fills result, request half byte-stable")
{
    ...
    const std::string md = ursa::tool_call_markdown(*done);
    CHECK(md.find("Requested to call:") == 0);
    CHECK(md.find("```bash\nls -la\n```") != std::string::npos);
    CHECK(md.find("---") != std::string::npos);
    CHECK(md.find("Rejected") == std::string::npos);

    CHECK(env.last_request().messages.back().content.find("ran: ls -la")
        != std::string::npos);
    CHECK(env.user_turn_count() == 1);
}
...
TEST_CASE("reject with reason reaches transcript and injected result")
{
    ...
    CHECK(ursa::tool_call_markdown(*tc).find("> Rejected: needs approval first")
        != std::string::npos);
...
TEST_CASE("esc on tool injects generic denial, appends nothing to transcript")
{
    ...
    const std::string md = ursa::tool_call_markdown(*tc);
    CHECK(md.find("---") == std::string::npos);
    CHECK(md.find("Rejected") == std::string::npos);
    CHECK(env.user_turn_count() == 1);
...
TEST_CASE("one drain cycle folds question answer and tool output correctly")
{
    ...
    CHECK(
        msgs[tool_idx].content.find("Requested to call:") != std::string::npos);
}
...
TEST_CASE("user modal enqueued mid-stream surfaces after the ask resolves")
{
    ...
    env.controller.enqueue_user_modal(ursa::SettingsModal { "m" });
    env.pump.pump();
