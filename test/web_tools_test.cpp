...
{
    const std::string two_byte = "a" "\xc3" "\xa9" "b";
    const std::string three_byte = "\xe6" "\x97" "\xa5";
    const std::string four_byte = "\xf0" "\x9f" "\x98" "\x80";

    CHECK(ursa::truncate_utf8(two_byte, 10) == two_byte);
    CHECK(ursa::truncate_utf8(two_byte, 1) == "a");
    CHECK(ursa::truncate_utf8(two_byte, 2) == "a");
    CHECK(ursa::truncate_utf8(two_byte, 3) == "a" "\xc3" "\xa9");
...
TEST_CASE("html_to_text leaves unknown entities and text intact")
{
    ...
    CHECK(ursa::html_to_text("<span>just</span> text")
          == "just text");
    CHECK(ursa::html_to_text("<p>line<br>break</p>")
          == "line\nbreak");
...
{
    const std::string json_response = R"({"jsonrpc":"2.0","id":1,"result":)"
...
TEST_CASE("webfetch rejects invalid arguments")
{
    ...
    const auto scheme = tool.run(
        ursa::parse_json(R"({"url":"ftp://example.com"})"));
