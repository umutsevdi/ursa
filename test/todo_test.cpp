...
TEST_CASE("parse_todo_args rejects malformed args")
{
    ...
    CHECK_FALSE(ursa::parse_todo_args(ursa::parse_json("{}")).has_value());
    CHECK_FALSE(ursa::parse_todo_args(ursa::parse_json(
                    R"json({"todos":"nope"})json"))
                    .has_value());
    CHECK_FALSE(ursa::parse_todo_args(
                    ursa::parse_json(R"json({"todos":[{}]})json"))
                    .has_value());
    CHECK_FALSE(ursa::parse_todo_args(
                    ursa::parse_json(R"json({"todos":[{"content":""}]})json"))
                    .has_value());
    CHECK_FALSE(
        ursa::parse_todo_args(
            ursa::parse_json(R"json({"todos":[{"content":"a","status":"done"}]})json"))
            .has_value());
}
