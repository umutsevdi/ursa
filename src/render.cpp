...
// Inline formatting flags carried alongside every text run.
struct Style {
    bool bold = false;
    bool emph = false;
    bool code = false;
    bool link = false;
    std::string url; // set while inside a link
};
...
void walk_markdown(cmark_node* doc, Sink& s)
{
    ...
    while ((ev = cmark_iter_next(iter)) != CMARK_EVENT_DONE) {
        ...
        if (enter) {
            ...
            case CMARK_NODE_LIST:
                lists.push_back(cmark_node_get_list_type(node) ==
                                        CMARK_ORDERED_LIST ? 1 : 0);
...
public:
    ...
    // Flexbox wrapping cannot represent hard breaks mid-paragraph; treat them
    // as soft breaks.
    ...
    void heading_end()
    {
        ...
        static const Color kHeadColors[] = {
            Color::White, Color::CyanLight, Color::MagentaLight,
            Color::YellowLight, Color::GreenLight, Color::BlueLight,
        };
    ...
    void code_block(std::string_view lit)
    {
        ...
        while (start <= lit.size()) {
            ...
            lines.push_back(
                ...
                color(Color::Palette256(245)) | bgcolor(Color::Palette256(234)));
            if (end == lit.size()) {
                break;
            }
            start = end + 1;
    ...
    void quote_end()
    {
        Element body = frames_.back().empty()
                           ? ftxui::text("")
                           : vbox(frames_.back());
        frames_.pop_back();
    ...
    {
        Element body =
            frames_.back().empty() ? ftxui::text("") : vbox(frames_.back());
        frames_.pop_back();
...
private:
    ...
    void push_words(std::string_view body, const Style& fl)
    {
        ...
        while (i < body.size()) {
            ...
            if (i < body.size()) {
                words_.push_back(ftxui::text(" ")); // wrap point
            }
        }
    }
    ...
    std::vector<Elements>    frames_;      // quote / item bodies
