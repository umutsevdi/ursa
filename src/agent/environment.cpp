...
namespace {
    ...
    void detect_git(Environment& env) { env.has_git = find_in_path("git"); }

    void detect_global_skills(Environment& env)
    {
            ...
            if (std::filesystem::exists(p)) {
                for (auto& skill : std::filesystem::directory_iterator(p)) {
                    if (skill.is_directory()
                        ...
                        env.global_skills.emplace(
                            skill.path().filename().string(),
                            skill.path() / "SKILL.md");
                    }
                }
            }
        ...
        if (std::filesystem::exists(skills_generic_cfg)) {
            for (auto& skill :
                ...
                if (skill.is_directory()
                    && std::filesystem::exists(skill.path() / "SKILL.md")) {
                    env.global_skills.emplace(skill.path().filename().string(),
                        skill.path() / "SKILL.md");
                }
            }
        }
    ...
    void detect_project_skills(
        const std::filesystem::path& root, Environment& env)
    {
        auto home_dir_skills = { ".opencode", ".claude", ".codex", ".grok",
            ".gemini", ".agents", ".cursor", ".openclaw" };
        for (const auto& skill_path : home_dir_skills) {
...
Environment analyze_environment()
{
    ...
    detect_git(env);
    detect_global_skills(env);
    std::error_code ec;
    const std::filesystem::path cwd = std::filesystem::current_path(ec);
    if (!ec) {
        env.instruction = load_agent_file(cwd);
        detect_project_skills(cwd, env);
    }
    return env;
}
