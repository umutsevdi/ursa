    ...
    SubagentCompleteFn complete, std::shared_ptr<Session> session)
{
    ...
    {
        std::lock_guard lock(mutex_);
        workers_.emplace_back(handle.id,
            std::jthread(
                [this, id = handle.id, promise, run = std::move(run),
                    ...
                    {
                        std::lock_guard lock(mutex_);
                        const auto task = std::find_if(tasks_.begin(),
                            tasks_.end(), [id](const SubagentTask& candidate) {
                    ...
                    changed_.publish(SubagentEvent { result.status == Status::OK
...
bool SubagentManager::cancel(std::size_t id)
{
    ...
    const auto worker = std::ranges::find(
        workers_, id, &std::pair<std::size_t, std::jthread>::first);
...
std::size_t SubagentManager::running_count(bool visible_only) const
{
    ...
    return static_cast<std::size_t>(std::count_if(
        tasks_.begin(), tasks_.end(), [visible_only](const SubagentTask& task) {
